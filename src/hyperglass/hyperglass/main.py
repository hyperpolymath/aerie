...
This is the main entry point for the hyperglass application. It handles
pre-start checks (Python/Node versions), builds the frontend, registers
plugins, and starts the Uvicorn ASGI server.
"""
...
# COMPATIBILITY CHECK: Python
# Ensure the Python version meets the minimum requirements defined in constants.py.
...
# Local imports for state and settings
from .util import cpu_count
from .state import use_state
from .settings import Settings

# GLOBAL LOGGING SETUP
# Set log level based on debug settings and initialize the intercept handler
# for third-party libraries (Paramiko, Scrapy, etc.).
...
async def build_ui() -> bool:
    ...
    This compiles the Next.js/Tailwind frontend located in src/hyperglass/docs
    (or as configured) and moves the assets to the static directory.
    """
    # Local
...
def register_all_plugins() -> None:
    ...
    Registers built-in, directive-based, and global plugins from the configuration.
    ...
    # Register built-in plugins (FRR, Juniper, Mikrotik, etc.).
    init_builtin_plugins()

    failures = ()

    # Register external directive-based plugins (defined in devices.yaml).
    ...
    # Register external global/common plugins (defined in hyperglass.yaml).
...
def unregister_all_plugins() -> None:
    """Unregister all plugins and reset managers."""
    # Local
...
def start(*, log_level: t.Union[str, int], workers: int) -> None:
    ...
    This function triggers the UI build (if enabled), registers plugins,
    and then hands off control to Uvicorn.
    """
...
def run(workers: int = None):
    """Primary application runner.

    Initializes the user configuration, sets up file and syslog logging,
    calculates worker counts, and starts the server.
    """
    # Local
    ...
    try:
        ...
        # Calculate worker count based on CPU if not specified
        _workers = workers
        if workers is None:
            if Settings.debug:
                _workers = 1
        ...
        log.critical(error)
        # Cleanup state on crash unless in dev mode
